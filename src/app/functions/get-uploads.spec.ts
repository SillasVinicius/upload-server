...
import { makeUploads } from '@/test/factories/make-upload'
...
describe('get uploads', () => {
  ...
  it('should be able to get sorted uploads', async () => {
    ...
    const upload1 = await makeUploads({ name: `${namePattern}.webp`, createdAt: new Date() });
    const upload2 = await makeUploads({ name: `${namePattern}.webp`, createdAt: dayjs().subtract(1, 'days').toDate() });
    const upload3 = await makeUploads({ name: `${namePattern}.webp`, createdAt: dayjs().subtract(2, 'days').toDate() });
    const upload4 = await makeUploads({ name: `${namePattern}.webp`, createdAt: dayjs().subtract(3, 'days').toDate() });
    const upload5 = await makeUploads({ name: `${namePattern}.webp`, createdAt: dayjs().subtract(4, 'days').toDate() });
